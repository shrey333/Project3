...
class aws_grader:
    def __init__(
        self, access_key, secret_key, input_bucket, output_bucket, lambda_name, region
    ):
        ...
        self.lambda_function = boto3_client(
            "lambda",
            aws_access_key_id=self.access_key,
            aws_secret_access_key=self.secret_key,
            region_name=region,
    ...
    def validate_lambda_exists(self, TC_num):
        try:
            response = self.lambda_function.get_function(FunctionName=self.lambda_name)
            print(
                f"Lambda function {self.lambda_name} HTTPStatusCode {response['ResponseMetadata']['HTTPStatusCode']}"
            )
    ...
    def validate_s3_subfolders(self, TC_num):
        ...
        for obj in in_objects["Contents"]:
            ...
            out_objects = self.s3.list_objects_v2(
                Bucket=self.out_bucket_name, Prefix=folder_name, Delimiter="/"
            )
    ...
    def validate_s3_output_objects(self, TC_num):
        ...
        try:
            ...
            print(
                f"Got {len(objects)} objects {[o.key for o in objects]} from bucket {bucket.name}"
            )
    ...
    def validate_s3_buckets_initial(self, TC_num):
        ...
        if out_isExist:
            ...
            print(
                f"S3 Output Bucket:{self.out_bucket_name} has {op_obj_count} object(s)"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--access_key", type=str, help="ACCCESS KEY ID of the grading IAM user"
    )
    parser.add_argument(
        "--secret_key", type=str, help="SECRET KEY of the grading IAM user"
    )
    parser.add_argument("--input_bucket", type=str, help="Name of the S3 Input Bucket")
    parser.add_argument(
        "--output_bucket", type=str, help="Name of the S3 Output Bucket"
    )
    ...
    aws_obj = aws_grader(
        access_key, secret_key, input_bucket, output_bucket, lambda_name, region
    )
    aws_obj.main()
